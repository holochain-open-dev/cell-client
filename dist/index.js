...
var async_with_timeout_1$1 = createCommonjsModule(function (module, exports) {
...
var async_with_timeout_1 = __importDefault(async_with_timeout_1$1);
                    ...
                    case 2:
                        ...
                        if (err_1.name === "TimeoutError") {
                            if (this.loaded) {
                                ...
                                throw new async_with_timeout_1$1.TimeoutError("Failed to complete COMB handshake", err_1.timeout);
                            }
                            else {
                                // log.error("iFrame did not trigger load event");
                                throw new async_with_timeout_1$1.TimeoutError("Failed to load iFrame", err_1.timeout);
                            }
                        }
                        else
                            throw err_1;
...
if (!TESTING)
  if (typeof window !== "undefined") window.COMB = build.COMB;
...
const WebSdkConnection = src.Connection;
class WebSdkClient extends BaseClient {
    constructor(url, branding) {
        super();
        this.connection = new WebSdkConnection(url, (s) => this.handleSignal(s), branding);
    }
}
class HoloClient {
    constructor(connection, cellData) {
    ...
    async callZome(zomeName, fnName, payload) {
        const result = await this.connection.connection.zomeCall(this.cellData.role_id, zomeName, fnName, payload);
...
export { HoloClient, HolochainClient, WebSdkClient };
